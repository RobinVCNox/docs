...
description: 'Guide on how to let your users connect their assets to NOX Energy from your website/app, whatever your business.'
---
...
### 1. User creation from the partner's website/app
...
4. **Done!** → NOX Energy has a `user_id`/`device_id` to `partner_user_id` mapping and data is available in our Package API endpoints.

![NOX Energy Partner Authentication Flow](/guides/images/supplier_auth_flow.png)

<sub>_Where these screens read "supplier", the same applies to your partner account._</sub>
...
To connect a **PV (solar) device**, set `"asset_type": "pv"`. The user will then only see PV inverter brands. To connect a **heat pump**, set `"asset_type": "hp"`. The `asset_type` is the type of asset your user is connecting, and it does **not** need to match your own product category. If you also pass a `brand` in the link-session request, then you will directly go to that brand's connect page instead of first arriving at the brand selector page. This can be useful if you want to have your own style of brand selector within your app. Note that the provided brand must be a valid brand for the chosen `asset_type`. See the [Supported Assets Overview](/supported-assets/supported-assets-overview) for the full list of brands per category.

Example, a solar OEM letting a user connect their heat pump:
```bash
curl --request POST \
