...
**API Packages:**
* **NOX control**: Option to integrate into your website/app + NOX Energy controls
...
Any combination of the following optimizations are possible:
 ...
 automatically shifting usage to periods with lower costs or when energy prices provide net benefits to users. 
  ...
  By default we optimize on day-ahead market and imbalance market. A separation of optimization between day-ahead and imbalance market is also possible.
* **Partner control**: Option to integrate into your website/app + NOX Energy provides 
real-time and forecasted energy flex bounds (aggregated or per asset), enabling partners to execute demand response 
actions when financially beneficial. NOX Energy handles device control, authentication and manufacturer partnerships, 
eliminating implementation complexity for partners but giving steering control. This allows the partner to execute their
...
integration for both NOX control and partner control optimizations. 
Furthermore, we provide and manage an authentication process for all integrated partners. Additionally, user or asset settings
