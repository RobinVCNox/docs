...
<Update label="August 2026">
  ...
  * The Authenticator now appends a `success` query parameter to your `redirect_url` when redirecting the user back: `?success=true` (alongside the existing base64 `data` payload) on success, or `?success=false` on failure — including when the user declines consent on the manufacturer's OAuth screen. This lets a single redirect URL render both outcomes without inspecting the `data` payload. See the [Energy Supplier Integration](/guides/energy-supplier-integration) guide. `redirect_url` must be a valid `http(s)` URL; any query params you include are preserved.
